...
!function ($) {
	...
	var create_slice = function(node, width, color) {
	    ...
	    var $change = $('<p class="amount-change" style="color:#cc0000; font-size:12px; margin-top:-15px;">&nbsp;</p>')
	...
	var update_slices = function( amount, contributions, indirect ) {
		$('.amount').each(function(i) {
		    var $slice = $(this);
		    var $slicechange = $(this).siblings('.amount-change');
		    ...
		    // the amount class
		    var old = parseInt($slice.text().replace(',',''));
		    $slice.text(
			OpenSpending.Amounts.format(
			...
			if (indirect) {
			    $slicechange.text(
				'(+'+OpenSpending.Amounts.format(
				    (slice_tax / config.salary.division)-old,
				    2, config.currency)+')'
			    );
			}
			else {
	 ...
	 var initialize = function() {
		 ...
		 if ($(this).attr('checked')) {
		     controls.salary(parseInt($('.salary .salaryamount').text().replace(',','')));
		 }
		 else {
		     console.log('Setting '+ controls.salarychange.text());
		     controls.salary(-1*parseInt(controls.salarychange.text().replace(',','').replace('(','')));
		 }
	    });
	...
	this.salarychange = $('<p class="salarychange" style="color:#cc0000; margin-top:-15px; font-size:12px;">-</p>');
	this.salary = function( value ) {
		...
		return $('<div class="salary" />')
		    ...
		    .append('<p class="salaryamount"></p>')
		    .append(this.salarychange);
	    }
	    else {
		if (this.checkbox.attr('checked')) {
		    $('.salary .salaryamount').text(
			OpenSpending.Amounts.format(0, 0, config.currency)
		    );
		    this.salarychange.text(
	...
	var taxamount = $('<p id="tax-amount"/>');
	var taxchange = $('<p id="tax-change" style="color:#cc0000; font-size:12px; margin-top:-15px;">&nbsp;</p>');
	this.tax = function( value, indirect ) {
	    if (!arguments.length) {
		...
		var old = parseInt(taxamount.text().replace(',',''));		
		...
		if (arguments.length == 2) {
		    if (indirect) {
			taxchange.text(
			    '(+'+OpenSpending.Amounts.format(value-old, 0, config.currency)+')'
			);
		    }
		    else {
			taxchange.html('&nbsp;');
